...
promedio = tprun("3", "nba_2016_equipoHacker.in", "nba_2016_equipoHacker.out")
promedioDf = pd.DataFrame(data={'promedio': promedio, 'nombres': (x for x in range(len(nombres)))})
obtenerOrden(promedioDf, "promedio")
...
rects2 = ax.bar(x, df.promedio, width, label='Promedio', color="red")
